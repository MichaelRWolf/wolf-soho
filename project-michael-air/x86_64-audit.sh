#!/usr/bin/env bash
# Intel x86_64 binary audit for michael-air
# Started 2026-08-15 as background process
# See github.com/MichaelRWolf/wolf-soho/issues/8

exec > x86_64-audit.log 2>&1

echo "=== Intel x86_64 Audit for michael-air ==="
echo "Started: $(date)"
echo

echo "## 1. Common package locations"
echo "### /usr/local binaries"
find /usr/local -type f -executable 2>/dev/null | xargs file 2>/dev/null | grep x86_64 | head -50

echo
echo "### /opt binaries"
find /opt -type f -executable 2>/dev/null | xargs file 2>/dev/null | grep x86_64 | head -50

echo
echo "### ~/.local binaries"
find ~/.local -type f -executable 2>/dev/null | xargs file 2>/dev/null | grep x86_64 | head -50
...
if [ -n "$python_paths" ]; then
  echo "$python_paths" | while read path; do
    if [ -d "$path" ]; then
      echo "### $path"
      find "$path" -name "*.so" -o -name "*.pyd" 2>/dev/null | xargs file 2>/dev/null | grep x86_64 | head -10 || echo "  (no x86_64 extensions)"
    fi
  done
fi
...
if which gem &>/dev/null; then
  ...
  find ~/.gem -name "*.so" 2>/dev/null | xargs file 2>/dev/null | grep x86_64 | head -10 || echo "  (no x86_64 gems)"
fi

echo
echo "## 6. /Applications bundles"
echo "### Scanning /Applications for x86_64 binaries (first 30)"
find /Applications -type f -executable 2>/dev/null | xargs file 2>/dev/null | grep x86_64 | head -30
...
if [ -d ~/Library/Developer/Xcode/DerivedData ]; then
  ...
  find ~/Library/Developer/Xcode/DerivedData -type f -executable 2>/dev/null | xargs file 2>/dev/null | grep x86_64 | head -10 || echo "  (none found)"
fi
...
if [ -d ~/.npm ]; then
  ...
  find ~/.npm -name "*.node" 2>/dev/null | xargs file 2>/dev/null | grep x86_64 | head -10 || echo "  (none found)"
fi
